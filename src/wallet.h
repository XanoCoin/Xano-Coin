...
// 8249 = 1*5000 + 1*1000 + 1*500 + 1*100 + 1*50 + 1*10 + 1*5 + 1
static const int ZQ_6110 = 8249;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 XANO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zXANO send
enum ZerocoinSpendStatus {
    ZXANO_SPEND_OKAY = 0,                            // No error
    ZXANO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXANO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXANO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXANO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXANO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXANO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXANO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXANO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXANO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXANO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXANO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXANO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXANO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXANO_SPENT_USED_ZXANO = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZXanoAutoBackups(bool fEnabled)
    {
