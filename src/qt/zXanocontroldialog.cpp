...
#include "ui_zXanocontroldialog.h"
...
std::list<std::string> ZXanoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXanoControlDialog::listMints;

ZXanoControlDialog::ZXanoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXanoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXanoControlDialog::~ZXanoControlDialog()
{
    delete ui;
}

void ZXanoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXanoControlDialog::updateList()
{
...
void ZXanoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXanoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXano_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXanoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXanoControlDialog::GetSelectedMints()
{
...
void ZXanoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
