                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXANO ought to be enough for anybody." - Bill Gates, 2017
    ui->zXANOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXANOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXANO->setEnabled(false);
        ui->pushButtonMintzXANO->setToolTip(tr("zXANO is currently disabled due to maintenance."));

        ui->pushButtonSpendzXANO->setEnabled(false);
        ui->pushButtonSpendzXANO->setToolTip(tr("zXANO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXANOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXANO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXANO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXANO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXANO in ") +
...
void PrivacyDialog::on_pushButtonSpendzXANO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXANO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXANO
        sendzXANO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXANO
    sendzXANO();
}

void PrivacyDialog::on_pushButtonZXanoControl_clicked()
{
    ZXanoControlDialog* zXanoControl = new ZXanoControlDialog(this);
    zXanoControl->setModel(walletModel);
    zXanoControl->exec();
}

void PrivacyDialog::setZXanoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXanoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXANO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Xano Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXANOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXANO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XANO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXANO</b>";
    ...
    // use mints from zXano selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXanoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXanoControlDialog::GetSelectedMints();
    }
    ...
    // Clear zXano selector in case it was used
    ZXanoControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXano Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Xano, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zXano Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXANO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXANO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXANO "));
    ui->labelzXANOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXANOSyncStatus->setVisible(fShow);
}
